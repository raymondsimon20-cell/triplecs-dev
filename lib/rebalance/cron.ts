...
import { appendInbox, listInbox, type AppendInput } from '../inbox';
...
import { DEFAULT_TARGETS } from '../utils';
...
export interface PerAccountRebalanceResult {
  accountHash: string;
  drift:       number;             // max drift %
  staged:      number;             // orders appended to inbox
  skipped?:    'low-drift' | 'existing-batch' | 'no-positions';
  error?:      string;
}

export interface RebalanceCronResult {
  ranAt:    string;                 // ISO timestamp
  accounts: PerAccountRebalanceResult[];
}

// ─── Helpers ─────────────────────────────────────────────────────────────────

/** Fetch every account + classify positions into pillars. */
async function fetchAccountSlices(): Promise<AccountSlice[]> {
...
function summarisePillars(slice: AccountSlice): PillarSummary[] {
...
const CANONICAL_BY_PILLAR: Record<string, string> = {
  triples:     'UPRO',
  cornerstone: 'CLM',
  income:      'JEPI',
  hedge:       'SPXU',
...
export async function runDriftRebalanceForAllAccounts(): Promise<RebalanceCronResult> {
  ...
  const pendingRebalance = await listInbox({ status: 'pending', source: 'rebalance' });
  ...
  const results: PerAccountRebalanceResult[] = [];
  ...
  for (const slice of slices) {
    ...
    try {
      const strategy = await getServerStrategyTargets(slice.accountHash);
      const pillars  = summarisePillars(slice);
      const targets  = targetMapFor({ ...DEFAULT_TARGETS, ...strategy });
      const maxDrift = Math.max(
        ...pillars
          .filter((p) => p.pillar !== 'other')
          .map((p) => Math.abs(p.portfolioPercent - (targets[p.pillar] ?? 0))),
        0,
      );
      ...
      const persisted = await appendInbox(orders);
      results.push({
        accountHash: slice.accountHash,
        drift:       maxDrift,

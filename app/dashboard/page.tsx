...
export default function DashboardPage() {
  ...
  const livePositions = useMemo(() => {
    ...
    const updated = positions.map((p) => {
      ...
      // marketValue alone leaves Value drifting live while Gain and Day stay
      // frozen at the server snapshot, so the columns disagree with each
      // other. Applying the delta preserves Schwab's basis-accurate gainLoss
      // rather than recomputing it from average cost.
      //
      // Note for the next person who sees Total Return jump: a +$2,353 move
      // here was briefly misread as this code trusting a bad price feed and
      // reverted. It was a 2% up day — on a book holding UPRO/TQQQ/SOXL that
      // is a +2.2% portfolio swing, which is exactly what showed up. Before
      // blaming the stream, difference the two days and check whether the
      // move is simply the market. Validating the feed means sampling a

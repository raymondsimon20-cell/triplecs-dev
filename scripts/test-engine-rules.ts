...
test('skips sub-$100 trims on small accounts barely above the cap', () => {
  ...
  assert.equal(fires.length, 0, `expected no CLM_CRF_TRIM signals on tiny account; got ${fires.length}`);
});
...
test('emits CLM+CRF trims when each half clears the $100 floor', () => {
  ...
  const result = runSignalEngine(baseInputs({
    positions: [
      enrichedPosition('CLM', 5000, 50_000),
      enrichedPosition('CRF', 5000, 50_000),
      enrichedPosition('SCHD', 100, 8_000),
    ],
    cash:       0,
  ...
  const fires = findSignals(result.signals, 'CLM_CRF_TRIM');
  const tickers = fires.map((s) => s.ticker).sort();
  assert.deepEqual(tickers, ['CLM', 'CRF'], `expected CLM+CRF SELLs; got ${tickers.join(',')}`);
  for (const f of fires) {
    assert.ok(f.sizeDollars >= 100, `${f.ticker} size $${f.sizeDollars} below floor`);
  }
});
...
test('skips when per-candidate size would be sub-$100 even with budget ≥ $100', () => {
  ...
  const result = runSignalEngine(baseInputs({
    positions: [
      ...
      enrichedPosition('SCHD', 70, 5_500), // cornerstone-ish
    ],
    cash:       1_000,
    marginDebt: 0,
